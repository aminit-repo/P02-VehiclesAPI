...
class CarController {
    ...
    ResponseEntity<CollectionModel<EntityModel<Car>>> list() {
        List<EntityModel<Car>> resources = carService.list().stream().map(assembler::toModel)
          ...
          CollectionModel<EntityModel<Car>> collection= CollectionModel.of(resources, linkTo(methodOn(CarController.class).list()).withSelfRel());
     ...
     */
    @PutMapping("/{id}")
    ResponseEntity<?> put(@PathVariable("id") Long id, @Valid @RequestBody Car car) {
        /**
